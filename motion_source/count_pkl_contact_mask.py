...
def process_motion(motion, cfg):
    ...
    # root_trans[:, :, 2] += 0.07    #开放此注释用于修改调整重定向时机器人与地面间的高度
    ...
    motion['root_pos'] = root_trans.squeeze(0).detach().numpy()
...
# --- drop-in patch: full shim + safer strategy order ---

def _ensure_numpy_core_shim():
    """Shim NumPy 2.x module paths so pickles saved under NumPy>=2 can load under NumPy 1.x."""
    import sys, types
    import numpy as _np
    ...
    if "numpy._core" not in sys.modules:
        ...
        core.__dict__.update(_np.__dict__)
        sys.modules["numpy._core"] = core

    # map submodules that pickles often reference
    try:
        import numpy.core.multiarray as _ncore_multiarray
    except Exception:
        _ncore_multiarray = None

    # numpy._core.multiarray -> numpy.core.multiarray
    if "numpy._core.multiarray" not in sys.modules:
        if _ncore_multiarray is not None:
            sys.modules["numpy._core.multiarray"] = _ncore_multiarray
        else:
            sys.modules["numpy._core.multiarray"] = types.ModuleType("numpy._core.multiarray")

    # numpy._core._multiarray_umath -> best-effort alias
    if "numpy._core._multiarray_umath" not in sys.modules:
        if _ncore_multiarray is not None:
            sys.modules["numpy._core._multiarray_umath"] = _ncore_multiarray
        else:
            sys.modules["numpy._core._multiarray_umath"] = types.ModuleType("numpy._core._multiarray_umath")

def _try_joblib_load(file_path):
    # """Try loading with joblib"""
    """Prefer joblib first (handles compressed pickles)."""
    _ensure_numpy_core_shim()
    return joblib.load(file_path)

def _try_pickle_load(file_path):
    """Fallback: standard pickle on file object."""
    _ensure_numpy_core_shim()
    with open(file_path, "rb") as f:
        return pickle.load(f)

def _try_numpy_compatible_load(file_path):
        """Last resort: bytes-level fix then pickle.loads (may fail on compressed joblib pickles)."""
        _ensure_numpy_core_shim()
        import pathlib
        p = pathlib.Path(file_path)
        raw = p.read_bytes()
        fixed = raw.replace(b"numpy._core", b"numpy.core")
        return pickle.loads(fixed)
