...
class MotionLibBase():
    ...
    def load_data(self, motion_file, min_length=-1, im_eval = False):
        if osp.isfile(motion_file):
            ...
            # Check if directory exists and contains motion files
            if not osp.exists(motion_file):
                raise FileNotFoundError(f"Motion file directory does not exist: {motion_file}")
            if len(self._motion_data_load) == 0:
                raise FileNotFoundError(f"No .pkl motion files found in directory: {motion_file}")
