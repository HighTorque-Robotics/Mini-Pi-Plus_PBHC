...
from humanoidverse.utils.math import wrap_to_pi
...
class LeggedRobotMotionTracking(LeggedRobotBase):
    def __init__(self, config, device):
        ...
        # Initialize noise_to_level curriculum
        if hasattr(self.config, 'noise_curriculum') and self.config.noise_curriculum.enable:
            self.noise_to_level = self.config.noise_for_sample.initial_noise_level
            logger.info(f"Noise curriculum enabled, initial noise level: {self.noise_to_level}")
        else:
            self.noise_to_level = self.config.noise_for_sample.initial_noise_level
            logger.info(f"Using fixed noise level: {self.noise_to_level}")
    ...
    def _init_tracking_config(self):
        ...
        self.dof_pos_cfg_lower_limit=torch.zeros(self.num_dof, dtype=torch.float32, device=self.device, requires_grad=False)
        self.dof_pos_cfg_upper_limit = torch.zeros(self.num_dof, dtype=torch.float32, device=self.device, requires_grad=False)
        for i in range(self.num_dof):
            self.dof_pos_cfg_lower_limit[i] = self.config.robot.dof_pos_lower_limit_list[i]
            self.dof_pos_cfg_upper_limit[i] = self.config.robot.dof_pos_upper_limit_list[i]
    ...
    def _init_buffers(self):
        ...
        self.pri_rel_fut_ref_motion_state_flat = torch.zeros(self.num_envs, self.config.obs.obs_dims.pri_rel_fut_ref_motion_state_flat, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _reset_tasks_callback(self, env_ids):
        ...
        if hasattr(self.config, 'noise_for_sample') and self.config.noise_for_sample.enable:
            self._update_noise_to_level_curriculum()
    
    def _update_soft_dynamic_correction(self):
        sdc_curr = self.config.soft_dynamic_correction.curriculum
        
        average_completion_ratio = self.end_time_ratio_buf.mean()
        
        down_threshold = sdc_curr.completion_ratio_down_threshold
        up_threshold = sdc_curr.completion_ratio_up_threshold
 
        
        if average_completion_ratio < down_threshold:
            self.sdc_alpha *= (1 + sdc_curr.degree)
        elif average_completion_ratio > up_threshold:
            self.sdc_alpha *= (1 - sdc_curr.degree)
        self.sdc_alpha = np.clip(self.sdc_alpha, sdc_curr.min_alpha, sdc_curr.max_alpha)

    def _update_noise_to_level_curriculum(self):
        """
        Update the noise_to_level curriculum based on completion ratio.
        Higher completion ratio means better performance, so we can increase noise (make it harder).
        Lower completion ratio means worse performance, so we decrease noise (make it easier).
        """
        noise_curr = self.config.noise_for_sample
        
        # Get current completion ratio
        average_completion_ratio = self.end_time_ratio_buf.mean()
        
        # Get thresholds
        down_threshold = noise_curr.completion_ratio_down_threshold
        up_threshold = noise_curr.completion_ratio_up_threshold
          
        if average_completion_ratio < down_threshold:
            # Performance is bad, decrease noise to make it easier
            self.noise_to_level -=  noise_curr.degree
        elif average_completion_ratio > up_threshold:
            # Performance is good, increase noise to make it harder
            self.noise_to_level += noise_curr.degree
        
        # Clip to valid range
        self.noise_to_level = np.clip(self.noise_to_level, 
                                     noise_curr.min_noise_level, 
                                     noise_curr.max_noise_level)
                                     
        # Log the current noise level for monitoring
        self.log_dict["noise_to_level"] = torch.tensor(self.noise_to_level, dtype=torch.float, device=self.device)

    def _update_reward_penalty_curriculum(self):
        """
        Update the penalty curriculum based on completion ratio instead of episode length.
        Override base class method to use motion completion ratio.
        """
        average_completion_ratio = self.end_time_ratio_buf.mean()
        
        if average_completion_ratio < self.config.rewards.reward_penalty_level_down_threshold:
            self.reward_penalty_scale *= (1 - self.config.rewards.reward_penalty_degree)
        elif average_completion_ratio > self.config.rewards.reward_penalty_level_up_threshold:
            self.reward_penalty_scale *= (1 + self.config.rewards.reward_penalty_degree)
    ...
    def _update_reward_limits_curriculum(self):
        """
        Update the reward limits curriculum based on completion ratio instead of episode length.
        Override base class method to use motion completion ratio.
        """
        ...
        if self.use_reward_limits_dof_pos_curriculum:
            if average_completion_ratio < curriculum_cfg.soft_dof_pos_curriculum_level_down_threshold:
                self.soft_dof_pos_curriculum_value *= (1 + curriculum_cfg.soft_dof_pos_curriculum_degree)
            elif average_completion_ratio > curriculum_cfg.soft_dof_pos_curriculum_level_up_threshold:
                self.soft_dof_pos_curriculum_value *= (1 - curriculum_cfg.soft_dof_pos_curriculum_degree)
        ...
        if self.use_reward_limits_dof_vel_curriculum:
            if average_completion_ratio < curriculum_cfg.soft_dof_vel_curriculum_level_down_threshold:
                self.soft_dof_vel_curriculum_value *= (1 + curriculum_cfg.soft_dof_vel_curriculum_degree)
            elif average_completion_ratio > curriculum_cfg.soft_dof_vel_curriculum_level_up_threshold:
                self.soft_dof_vel_curriculum_value *= (1 - curriculum_cfg.soft_dof_vel_curriculum_degree)
        ...
        if self.use_reward_limits_torque_curriculum:
            if average_completion_ratio < curriculum_cfg.soft_torque_curriculum_level_down_threshold:
                self.soft_torque_curriculum_value *= (1 + curriculum_cfg.soft_torque_curriculum_degree)
            elif average_completion_ratio > curriculum_cfg.soft_torque_curriculum_level_up_threshold:
                self.soft_torque_curriculum_value *= (1 - curriculum_cfg.soft_torque_curriculum_degree)
    ...
    def _update_terminate_when_dof_far_curriculum(self):
        ...
        down_threshold = dof_far_curr.completion_ratio_down_threshold
        up_threshold = dof_far_curr.completion_ratio_up_threshold
    ...
    def _update_terminate_when_motion_far_curriculum(self):
        ...
        down_threshold = self.config.termination_curriculum.terminate_when_motion_far_completion_ratio_down_threshold
        up_threshold = self.config.termination_curriculum.terminate_when_motion_far_completion_ratio_up_threshold
    ...
    def _update_reset_buf(self):
        ...
        if self.config.termination.terminate_when_motion_far:
            terminate_when_motion_far_threshold = self.terminate_when_motion_far_threshold+self.noise_to_level*0.3
            reset_buf_motion_far = torch.any(torch.norm(self.dif_global_body_pos, dim=-1) >terminate_when_motion_far_threshold, dim=-1)
            ...
            if self.config.termination_curriculum.terminate_when_motion_far_curriculum:
                self.log_dict["terminate_when_motion_far_threshold"] = torch.tensor(terminate_when_motion_far_threshold, dtype=torch.float)
    ...
    def _resample_motion_times(self, env_ids):
        ...
        # print(f"self.motion_ids[env_ids]================== {self.motion_ids[env_ids]}")
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.custom_origins: # trimesh
            ...
            # Sample random noise level between 0 and self.noise_to_level for each environment
            root_pos_noise_sample = torch.rand(len(env_ids), device=self.device) * self.noise_to_level
            root_rot_noise_sample = torch.rand(len(env_ids), device=self.device) * self.noise_to_level
            root_vel_noise_sample = torch.rand(len(env_ids), device=self.device) * self.noise_to_level
            root_ang_vel_noise_sample = torch.rand(len(env_ids), device=self.device) * self.noise_to_level
            root_pos_noise = self.config.init_noise_scale.root_pos * root_pos_noise_sample.unsqueeze(-1)
            root_rot_noise = self.config.init_noise_scale.root_rot * 3.14 / 180 * root_rot_noise_sample
            root_vel_noise = self.config.init_noise_scale.root_vel * root_vel_noise_sample.unsqueeze(-1)
            root_ang_vel_noise = self.config.init_noise_scale.root_ang_vel * root_ang_vel_noise_sample.unsqueeze(-1)
    ...
    def small_random_quaternions(self, n, max_angle):
            ...
            # Handle both scalar and vector max_angle inputs
            if torch.is_tensor(max_angle) and max_angle.numel() > 1:
                # max_angle is a vector, reshape to (n, 1) for broadcasting
                angles = max_angle.view(-1, 1) * torch.rand((n, 1), device=self.device)
            else:
                # max_angle is a scalar
    ...
    def _reset_dofs(self, env_ids):
        ...
        # Sample random noise level between 0 and self.noise_to_level for each environment
        dof_pos_noise_sample = torch.rand(len(env_ids), device=self.device) * self.noise_to_level
        dof_vel_noise_sample = torch.rand(len(env_ids), device=self.device) * self.noise_to_level
        dof_pos_noise = self.config.init_noise_scale.dof_pos * dof_pos_noise_sample.unsqueeze(-1)
        dof_vel_noise = self.config.init_noise_scale.dof_vel * dof_vel_noise_sample.unsqueeze(-1)
    ...
    def kick_motion_res_multiplestep(self) -> Dict[str, torch.Tensor]:
        ...
        for i in range(self.config.obs.future_ref_steps):
            # In evaluation mode, use fixed time sequence starting from motion start time
            if self.is_evaluating:
                motion_times = (self.episode_length_buf + 1 + i) * self.dt 

                offset = torch.zeros_like(self.env_origins)  # Use zero offset in eval for consistency
            else:
    ...
    def _pre_compute_observations_callback(self):
        ...
        if "future_ref_steps" in self.config.obs and self.config.obs.future_ref_steps > 0:
            ...
            # Extract additional future reference data for _get_obs_rel_fut_ref_motion_state_flat
            self.ref_dof_pos_fut = future_ref_joint_pos  # [num_envs, num_future_steps, num_dofs]
            self.ref_dof_vel_fut = future_ref_joint_vel  # [num_envs, num_future_steps, num_dofs]
            self.ref_body_pos_fut = future_motion_res["rg_pos_t"]  # [num_envs, num_future_steps, num_bodies_extend, 3]
            self.ref_body_rot_fut = future_motion_res["rg_rot_t"]  # [num_envs, num_future_steps, num_bodies_extend, 4]
            self.ref_body_vel_fut = future_motion_res["body_vel_t"]  # [num_envs, num_future_steps, num_bodies_extend, 3]
            self.ref_body_ang_vel_fut = future_motion_res["body_ang_vel_t"]  # [num_envs, num_future_steps, num_bodies_extend, 3]
            self.ref_base_rot_fut = future_motion_res["root_rot"]  # [num_envs, num_future_steps, 4]
            self.ref_base_lin_vel_fut = future_motion_res["root_vel"]  # [num_envs, num_future_steps, 3]
            self.ref_base_ang_vel_fut = future_motion_res["root_ang_vel"]  # [num_envs, num_future_steps, 3]
            #  (Pdb) motion_res.keys()
        ...
        # Store current reference data for transformations in _get_obs_rel_fut_ref_motion_state_flat
        self.ref_root_global_pos_t = motion_res["root_pos"]  # [num_envs, 3]
        self.ref_root_global_rot_quat_t = motion_res["root_rot"]  # [num_envs, 4]
        
        # Store reference initial heading for transformations
        if not hasattr(self, 'ref_initial_heading_inv_quat'):
            if self.is_evaluating:
                motion_times_init = torch.zeros_like(self.motion_start_times)
                if hasattr(self, 'env_origins'):
                    offset_init = self.env_origins
                else:
                    offset_init = torch.zeros((self.num_envs, 3), device=self.device)
                motion_res_init = self._motion_lib.get_motion_state(self.motion_ids, motion_times_init, offset=offset_init)
                self.ref_initial_heading_inv_quat = calc_heading_quat_inv(motion_res_init["root_rot"], w_last=True)
            else:
                self.ref_initial_heading_inv_quat = calc_heading_quat_inv(motion_res["root_rot"], w_last=True)
        ...
        # Store robot body positions for transformations
        self._robot_rel_body_pos_t = self._rigid_body_pos_extend
        ...
        self.pri_rel_fut_ref_motion_state_flat = self._get_pri_rel_fut_ref_motion_state_flat()
    ...
    def _get_pri_rel_fut_ref_motion_state_flat(self):
        """
        Get relative future reference motion state flattened.
        Based on holo3 implementation but adapted for fut data structure.
        """
        if not hasattr(self, 'ref_body_pos_fut'):
            # Return zeros if future data is not available
            return torch.zeros((self.num_envs, 0), device=self.device)
            
        num_bodies_extend = self._robot_rel_body_pos_t.shape[1]
        num_fut_timesteps = self.ref_body_pos_fut.shape[1]

        # Calculate future reference RPY
        ...
        # Calculate future yaw relative to initial reference heading
        ...
        _, _, ref_fut_yaw_flat = get_euler_xyz(
        ...
        # Get current reference base data for transformations
        ...
        # Transform future body positions to local frame
        ...
        # Transform future body rotations to local frame
        ...
        # Transform future body velocities to local frame
        ...
        # Transform future body angular velocities to local frame
        ...
        # Transform future base velocities to local frame
        cur_ref_base_rot_heading_quat_inv_fut_flat = (
            cur_ref_base_rot_heading_quat_inv[:, None, :]
            .repeat(1, num_fut_timesteps, 1)
            .reshape(-1, 4)
        )
        ...
        # Concatenate all future reference motion data
        rel_fut_ref_motion_state_flat = torch.cat(
            [
                ...
                fut_rel_ref_bodylink_pos_flat,  # [B, T * num_bodies_extend * 3]
                fut_rel_ref_bodylink_rot_tannorm_flat,  # [B, T * num_bodies_extend * 6]
                fut_rel_ref_bodylink_vel_flat,  # [B, T * num_bodies_extend * 3]
                fut_rel_ref_bodylink_ang_vel_flat,  # [B, T * num_bodies_extend * 3]
            ],
            dim=-1,
        )
        # self.rel_fut_ref_motion_state_flat = rel_fut_ref_motion_state_flat

        return rel_fut_ref_motion_state_flat
    def _get_obs_pri_rel_fut_ref_motion_state_flat(self):
        return self.pri_rel_fut_ref_motion_state_flat
    
    def _get_obs_rel_fut_ref_motion_state_flat(self):
        return self.pri_rel_fut_ref_motion_state_flat[:, 3:]
    ...
    def _reward_limits_dof_pos(self):
        ...
        if self.use_reward_limits_dof_pos_curriculum:
            m = (self.dof_pos_cfg_lower_limit + self.dof_pos_cfg_upper_limit) / 2
            r = self.dof_pos_cfg_upper_limit - self.dof_pos_cfg_lower_limit
            lower_soft_limit = m - 0.5 * r * self.soft_dof_pos_curriculum_value
            upper_soft_limit = m + 0.5 * r * self.soft_dof_pos_curriculum_value
        else:
            lower_soft_limit = self.dof_pos_cfg_lower_limit
            upper_soft_limit = self.dof_pos_cfg_upper_limit
